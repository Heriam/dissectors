...
//                opt_offset += 1;
                ...
                switch(opt_len){
                    case 1: /* Default Option Length without prefix */
                        proto_item_append_text(ti, " ::/%d", prefix_len);
                        break;
                    case 7:
                        ...
                        proto_item_append_text(ti, " %s(prefix len: %d)", address_to_str(wmem_packet_scope(), &target_prefix_addr), prefix_len);
                        opt_offset += 8;
                        break;
                    case 17:
                        proto_tree_add_item(icmp6opt_tree, hf_icmpv6_rpl_opt_target_prefix, tvb, opt_offset, 16, ENC_NA);
                        proto_item_append_text(ti, " %s(prefix len: %d)", tvb_ip6_to_str(tvb, opt_offset), prefix_len);
                        opt_offset += 16;
                        break;
                    default:
                        expert_add_info(pinfo, ti_opt_len, &ei_icmpv6_invalid_option_length);
            ...
            case RPL_OPT_TRANSIT: {
                struct e_in6_addr   transit_prefix;
                address transit_prefix_addr;
...
//                opt_offset += 1;
                ...
                switch(opt_len){
                    case 1:
                        break;
                    case 9:
                        memset(&transit_prefix, 0, sizeof(transit_prefix));
                        tvb_memcpy(tvb, (guint8 *)&transit_prefix.bytes, opt_offset, 8);
                        proto_tree_add_ipv6(icmp6opt_tree, hf_icmpv6_rpl_opt_transit_parent, tvb, opt_offset, 8, &transit_prefix);
                        SET_ADDRESS(&transit_prefix_addr, AT_IPv6, 16, transit_prefix.bytes);
                        proto_item_append_text(ti, " %s", address_to_str(wmem_packet_scope(), &transit_prefix_addr));
                        opt_offset += 8;
                        break;
                    case 17:
...
dissect_rpl_control(tvbuff_t *tvb, int rpl_offset, packet_info *pinfo _U_, proto_tree *icmp6_tree, guint8 icmp6_type _U_, guint8 icmp6_code)
{
    ...
    switch(icmp6_code){
        ...
        {
            guint8 flags;
            struct e_in6_addr   dodagid;
//            address dodagid_addr;
...
//            rpl_offset += 1;
            ...
            {
                memset(&dodagid, 0, sizeof(dodagid));
                tvb_memcpy(tvb, (guint8 *)&dodagid.bytes, rpl_offset, 8);
                proto_tree_add_ipv6(icmp6_tree, hf_icmpv6_rpl_dao_dodagid, tvb, rpl_offset, 8, &dodagid);
//                SET_ADDRESS(&transit_prefix_addr, AT_IPv6, 16, transit_prefix.bytes);
                rpl_offset += 8;
            }
            /* Options */
